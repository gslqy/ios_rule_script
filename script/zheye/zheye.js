		...
		try {
			...
			if (!!obj["data"]) {
				...
				if (obj["paging"]["is_end"] === true) {
					$.notification.post(`同步黑名单数据成功！当前黑名单共${Object.keys(customBlockedUsers).length-defaultAnswerBlockedUsers.length}人。\n脚本内置黑名单${defaultAnswerBlockedUsers.length}人。`);
...
function removeMoments() {
	...
	try {
		...
		data = obj.data.filter(item => {
			const isBlackUserPin = settings_blocked_users && item.target && item.target["origin_pin"] && item.target["origin_pin"].author && typeof customBlockedUsers[item.target["origin_pin"].author.name] != "undefined";
	...
	try {
		if (!!$.response.body) {
			...
			obj.data.data.forEach(element => {
				const isAd = element["resource_type"] === "ad" || element["extra"]["recommend_start"] === true || element["next_guide"]["change_type"] === 1 || element["next_guide"]["title"] === "推荐的相关内容";
				if (isAd === true) {
					return
				}
				element["ad_info"] = {
...
function modifyAnswersNextRender() {
	...
	try {
		if (!!$.response.body) {
			...
			let newData = [];
			obj.data.forEach(element => {
				const isAd = element["ad_info"] && element["ad_info"]["data"] !== "" || element["biz_type_list"] && element["biz_type_list"].length !== 1 || element["biz_type_list"] && element["biz_type_list"][0] !== "answer" || element["type"] === "ad";
				if (isAd === true) {
					return
				}
	...
	if ($.isResponse) {
		switch (true) {
			...
			case $.data.read("zhihu_settings_app_conf", false) === true && /^https?:\/\/appcloud2\.zhihu\.com\/v\d+\/config/.test($.request.url):
				...
				break;
			case $.data.read("zhihu_settings_app_conf", false) === true && /^https?:\/\/m-cloud\.zhihu\.com\/api\/cloud\/config\/all\?/.test($.request.url):
				response = modifyMCloudConfig();
				break;
			case $.data.read("zhihu_settings_app_conf", false) === true && /^https?:\/\/api\.zhihu\.com\/feed\/render\/tab\/config/.test($.request.url):
			...
			case /^https?:\/\/api\.zhihu\.com\/next-render\?/.test($.request.url):
				...
				break;
			case $.data.read("zhihu_settings_sys_msg", true) !== false && /^https?:\/\/api\.zhihu\.com\/notifications\/v3\/message/.test($.request.url):
			...
			case /^https?:\/\/www\.zhihu\.com\/api\/v4\/comment_v5\/answers\/\d+\/abstract_comment\?/.test($.request.url):
				response = removeComment();
				break;
			case $.data.read("zhihu_settings_answer_tip", true) === true && /^https?:\/\/www\.zhihu\.com\/appview\/v2\/answer\/.*(entry=(?!(preload-topstory|preload-search|preload-subscription)))?/.test($.request.url):
			...
			case $.data.read("zhihu_settings_blocked_users", false) !== false && /^https?:\/\/api\.zhihu\.com\/next\?/.test($.request.url):
				...
				break;
			case $.data.read("zhihu_settings_blocked_users", true) === true && /^https?:\/\/api\.zhihu\.com\/people\/((?!self).)*$/.test($.request.url):
				response = autoInsertBlackList();
				break;
			case /^https?:\/\/api\.zhihu\.com\/moments_v3\?/.test($.request.url):
				response = removeMoments();
				break;
			case $.data.read("zhihu_settings_hot_list", true) === true && /^https?:\/\/api\.zhihu\.com\/topstory\/hot-lists(\?|\/)/.test($.request.url):
				response = removeHotListAds();
				break;
			case $.data.read("zhihu_settings_preset_words", true) === true && /^https?:\/\/api\.zhihu\.com\/search\/preset_words\?/.test($.request.url):
				response = removeKeywordAds();
				break;
			case $.data.read("zhihu_settings_blocked_users", false) !== false && /^https?:\/\/api\.zhihu\.com\/settings\/blocked_users/.test($.request.url):
				manageBlackUser();
				break;
			default:
		...
		if ($.data.read("zhihu_settings_blocked_keywords", false) !== false && /^https?:\/\/api\.zhihu\.com\/feed-root\/block/.test($.request.url) === true) {
